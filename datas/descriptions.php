<?php
$descriptions = [];

/* PROJETS */

$descriptions['mairie'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Stage de seconde année<br/>
<br/>
Dans le cadre de ce stage, j'ai développé un site web permettant aux agents de la mairie de Fleury-les-Aubrais qui étaient d'astreinte : 
<ul>
    <li>d’accéder à des documents (des procédures, des plans,…)</li>
    <li>d'envoyer un rapport de leur intervention</li>
    <li>de consulter le planning des personnes d’astreintes pour une semaine</li>
    <li>de consulter la liste des rapports des interventions effectués</li>
    <li>pour les personnes autorisées, de gérer le planning d'astreinte par semaine</li>
</ul>";


$descriptions['lafleur'] = "<b>Modalité :</b> Effectué en équipe<br/>
<b>Cadre :</b> Projet de seconde année<br/>
";


$descriptions['stage'] = "<b>Modalité :</b> Effectué en équipe<br/>
<b>Cadre :</b> Projet de fin de seconde année<br/>
<br/>
Dans le cadre de ce projet, mon avons poursuivi le développement d'un site qui avait été commencé par les précédents étudiants de BTS SIO mais qui était non fonctionnel et inachevé.<br/>
Le site devait permettre à chaque élève principalement :
<ul>
    <li>d'ajouter ses demandes de stage</li>
    <li>d'indiquer l'évolution de ses demandes</li>
    <li>de transmettre aux professeurs son attestation, sa convention et son évaluation de stage</li>
    <li>d'accéder à une liste d'entreprises déjà solicitées par le passé pour l'aider dans sa recherche de stage</li>
</ul>
Il devait également notamment permettre au professeur d'accéder au suivi des demandes de stage de chaque élève et, en début d'année, de paramétrer les périodes de stage.";


$descriptions['bucket'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Travail pratique<br/>
<br/>
Dans le cadre de travaux pratiques, j'ai développé, à l'aide du framework Symfony, un site permettant à un utilisateur d'écrire une idée d'activité à faire, pour que celle-ci soit ensuite partagée dans une liste publique contenant les idées de toutes les personnes inscrites sur le site.";


$descriptions['msl'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Stage de première année<br/>
<br/>
Dans le cadre de ce stage, j'ai développé un site à destination des employés de MSL Circuits.";


$descriptions['gitlab'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Travail pratique<br/>";


$descriptions['groupe_AD'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Travail pratique<br/>
Procédure de la gestion des stratégies de groupe dans Active Directory";


$descriptions['portfolio'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Préparation de l'examen<br/>";


$descriptions['glpi'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Travail pratique<br/>";



$descriptions['linkedin'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Préparation de mon projet professionnel<br/>
<br/>
Création d'un compte professionnel Linkedin.";


$descriptions['github'] = "<b>Modalité :</b> Effectué seul<br/>
<b>Cadre :</b> Préparation de mon projet professionnel<br/>
<br/>
Création d'un compte professionnel gitHub et importation de mes diffrents projets sur mon compte.";


/* FORMATIONS */

$descriptions['pix'] = "";

$descriptions['form_devops'] = "";


?>
